...
export function PortalShell({ children }: { children: React.ReactNode }) {
  ...
  // Same "unread" pattern, for Warranty Management — clears whenever the
  // Warranty page is opened, so a claim staff just approved/rejected/
  // reimbursed shows up as unread until the dealer actually looks at it.
  useEffect(() => {
    let active = true
    let since = ''
    try {
      since = localStorage.getItem(WARRANTY_LAST_SEEN_KEY) ?? ''
    } catch {
      // private-mode/unavailable storage — treat as never seen
    }
...
function PortalNavLink({ href, label, icon: Icon, active, onClick, badgeCount, asModule }: NavItem & { active?: boolean; onClick?: () => void; badgeCount?: number }) {
  const badge = !!badgeCount && (
    <span
      ...
      title={`${badgeCount} update${badgeCount === 1 ? '' : 's'} you haven't looked at yet`}
    >
